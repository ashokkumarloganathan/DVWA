...
# Install dependencies (php extensions + utils + curl for cloudflared)
RUN apt-get update \
...
# Install Composer (for API dependencies)
COPY --from=composer:latest /usr/bin/composer /usr/local/bin/composer

# Copy DVWA source code
...
# Install Cloudflared (latest stable)
...
# Start Apache and Cloudflare Tunnel together
CMD service apache2 start && cloudflared tunnel --no-autoupdate run --token ${TUNNEL_TOKEN}
